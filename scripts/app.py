...
user_input = st.sidebar.number_input('ID do Aluno (Usuário):', min_value=1, max_value=max_user_id, value=1250)
...
if st.sidebar.button('Gerar Recomendações', type="primary"):
    with st.spinner(f'Gerando as Top-{n_recommendations} recomendações para o Aluno {user_input}...'):
        recs_df = get_top_n_recommendations(model_svd, opt_data, maps, user_input, n_recommendations)
...
if not st.session_state.recommendations_df.empty:
    ...
    st.caption("A 'Nota Prevista' é a estimativa do modelo.")
else:
    st.info("👈 Use o painel à esquerda para selecionar um ID de Aluno e gerar recomendações.")
    st.markdown("---")
    st.image("https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExbndiaGZjOHlxejE3Zzl1bGJhNGRtZjJuZXVtOWpieTBqYzlzdnI4YSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/KX5nwoDX97AtPvKBF6/giphy.gif", caption="Processando Dados para Gerar Recomendações", width=300)
